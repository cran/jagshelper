...
-   The function `kfold()` provides automated k-fold or leave-one-out cross validation for a specified component of a JAGS data object, for a specified JAGS model.
...
may not matter.  However, interval bars may also be plotted with respect
to a quantitative variable as supplied by a numeric vector input.  
`caterpillar()` plots may be oriented vertically (default) or horizontal
(shown below).
